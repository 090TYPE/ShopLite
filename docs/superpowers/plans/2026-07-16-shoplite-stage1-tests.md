...
- [x] `dotnet test ShopLite.sln` из каталога ShopLite — зелёный, 42 теста. Проверено на чистой пересборке: 0 предупреждений, 0 ошибок.
- [x] Каждый эндпоинт обоих API имеет минимум один интеграционный тест: `POST /api/auth/register`, `POST /api/auth/login`, `POST /api/orders`, `GET /api/orders`, `GET /api/orders/{id}`.
- [x] Покрытие `OrderService.Domain` + `Order` — 60–70%. **Перевыполнено:** домен 100%, `Order` 98.1% (строчное по репозиторию — 96.6%, ветвлений — 91.6%). Цель оказалась заниженной: домен маленький и весь состоит из правил, каждое из которых по определению имеет тест.
- [x] Заказ с пустыми позициями, нулевым количеством или отрицательной ценой отклоняется с 400 — подтверждено тремя отдельными тестами.
- [x] E2E доказывает доставку `OrderCreated` через настоящий RabbitMQ. Проверено фальсификацией: при перенаправлении фабрики на мёртвый порт тест не зеленеет — брокер несущий, in-memory-подмены нет.
...
- Тесты `NotificationService`: консьюмер только пишет в лог — наблюдать нечего, пока туда не приедет реальная логика (например, отправка почты). E2E его **не** покрывает: проект на `NotificationService` не ссылается, а подписывается на то же событие своим `OrderCreatedSpy`. E2E доказывает, что событие проходит через настоящий брокер до консьюмера, — но не то, что работает именно `OrderCreatedConsumer`. Его покрытие — ноль, и это осознанно.
